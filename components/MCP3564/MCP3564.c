...
#define CLOCK_FREQ_HZ 2000000 // 2 MHz
    ESP_LOGI(TAG, "Configuring LEDC timer for clock generation");
    ...
    ESP_LOGI(TAG, "LEDC configuration complete. 20MHz clock started on GPIO %d", ADC_MCLK);
}
...
uint32_t readDataRegister()
{
    ...
    SPI->user.usr_mosi = 0;  // No MOSI phase
    SPI->user.usr_miso = 1;  // MISO phase
    SPI->ms_dlen.ms_data_bitlen = 31;  // 32 bits (0-31)
...
//    *mcp_obj->history = (uint32_t*)&history;
    ...
    ESP_LOGI(TAG, "GAINCAL = %lx", readSingleRegister(((_GAINCAL_ << 2) | _RD_CTRL_)));

    //OFFSETCAL --> 0
    writeSingleRegister(((_OFFSETCAL_ << 2) | _WRT_CTRL_), 0x000000);
    ESP_LOGI(TAG, "OFFSETCAL = %lx", readSingleRegister(((_OFFSETCAL_ << 2) | _RD_CTRL_)));

    //TIMER --> Disabled.
    writeSingleRegister(((_TIMER_ << 2) | _WRT_CTRL_), 0x000000);
    ESP_LOGI(TAG, "TIMER = %lx", readSingleRegister(((_TIMER_ << 2) | _RD_CTRL_)));

    //SCAN --> CH0 - CH5 --> ().
    writeSingleRegister(((_SCAN_ << 2) | _WRT_CTRL_), 0x00003F);
    ESP_LOGI(TAG, "SCAN = %lx", readSingleRegister(((_SCAN_ << 2) | _RD_CTRL_)));

    //MUX --> VIN+ = CH0, VIN- = CH1 --> (0b00000001).
    writeSingleRegister(((_MUX_ << 2) | _WRT_CTRL_), 0xBC);
    ESP_LOGI(TAG, "MUX = %lx", readSingleRegister(((_MUX_ << 2) | _RD_CTRL_)));

    //IRQ --> IRQ Mode = Low level active IRQ Output  --> (0b00000100).
    writeSingleRegister(((_IRQ_ << 2) | _WRT_CTRL_), 0x04);
    ESP_LOGI(TAG, "IRQ = %lx", readSingleRegister(((_IRQ_ << 2) | _RD_CTRL_)));

    //CONFIG3 --> Conv. Mod = One-Shot Conv. Mode, FORMAT = 32b + chid,
    //CRC_FORMAT = 16b, CRC-COM = Disabled,
    //OFFSETCAL = Disabled, GAINCAL = Disabled --> (0b10110000). 
    writeSingleRegister(((_CONFIG3_ << 2) | _WRT_CTRL_), 0xF0);
    ESP_LOGI(TAG, "CONFIG3 = %lx", readSingleRegister(((_CONFIG3_ << 2) | _RD_CTRL_)));
    
    //CONFIG2 --> BOOST = 1x, GAIN = 1x, AZ_MUX = 0 --> (0b10001011).
    writeSingleRegister(((_CONFIG2_ << 2) | _WRT_CTRL_), 0x89);
    ESP_LOGI(TAG, "CONFIG2 = %lx", readSingleRegister(((_CONFIG2_ << 2) | _RD_CTRL_)));

    //CONFIG1 --> AMCLK = MCLK, OSR = 64 --> (0b00000100).      
    writeSingleRegister(((_CONFIG1_ << 2) | _WRT_CTRL_), 0x04);
    ESP_LOGI(TAG, "CONFIG1 = %lx", readSingleRegister(((_CONFIG1_ << 2) | _RD_CTRL_)));
    ...
    ESP_LOGI(TAG, "CONFIG0 = %lx", readSingleRegister(((_CONFIG0_ << 2) | _RD_CTRL_)));
